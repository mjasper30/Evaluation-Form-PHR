...
function loadScores() {
  ...
  $("#scoresTable").DataTable({
    ...
    columns: [
      { width: "20px", data: "score_id" }, // Set width of 20 pixels for the first column
      { width: "200px", data: "username" }, // Set width of 200 pixels for the second column
      { width: "200px", data: "score" }, // Set width of 200 pixels for the second column
      { width: "150px", data: "time_finished" }, // Set width of 150 pixels for the third column
      {
        data: null,
        width: "100px",
        render: function (data, type, row) {
          var scoreID = row.score_id;
          return (
            '<button class="editScoreBtn btn btn-warning" data-score-id="' +
            scoreID +
            '" value="' +
            scoreID +
            '"><i class="bi bi-pencil-square"></i></button>' +
            '<button class="deleteScoreBtn ms-2 btn btn-danger" data-score-id="' +
            scoreID +
            '" value="' +
            scoreID +
            '"><i class="bi bi-trash"></i></button>'
    ...
    columnDefs: [
      { targets: [], orderable: false }, // Hide sorting for columns 1 and 3
    ],
  });
}
